...
metadata:
  ...
  related-skills: code-reviewer, typescript-code-review, cleanup-dead-code
---

# Publish Polish

Final audit for making a personal TypeScript/Node.js project ready for open-source publication. Produces a structured readiness report with a clear verdict.
...
1. **Privacy & Security Sweep** — Scan for secrets, credentials, API keys, hardcoded paths, personal information, and private data in the working tree. Search git history for accidentally committed secrets. Review `.gitignore` for completeness against canonical Node.js/TypeScript patterns. Load `references/repo-checklist.md` for scanning patterns. **This step is non-negotiable — mistakes here cannot be undone after push.**

2. **License & Legal** — Verify a LICENSE file exists at repo root with a recognized open-source license. Check that `package.json` `license` field matches if present. If no license exists, recommend one (MIT for permissive, Apache 2.0 if patent grant matters, GPL if copyleft desired).

3. **Code Polish** — Run formatting and lint checks. Audit TODO/FIXME/HACK comments. Check TSDoc/JSDoc on public exports. Verify TypeScript strict mode is enabled. Verify tests exist and pass. Load `references/code-quality.md` for detailed criteria.
...
5. **Packaging & Dependencies** — Check `package.json` for completeness:
   - `name`, `version`, `description`, `license`, `main`/`types`/`exports` fields populated
   - `dependencies` match actual imports in source
   - `devDependencies` contain dev-only packages (typescript, eslint, prettier, jest/vitest, etc.)
   - `engines` field specifies minimum Node.js version
   - `package-lock.json` or `yarn.lock` present and committed
   - `tsconfig.json` exists with `strict: true`
   - Cross-reference declared dependencies against actual `import`/`require` statements in source
   - Confirm no dev-only dependencies leak into `dependencies`
...
- Verify `.gitignore` covers: `node_modules/`, `.env`, `dist/`, `coverage/`, `*.tsbuildinfo`, IDE configs (`.vscode/`, `.idea/`), AI tooling (`.<harness>/`, `AGENTS.md`)
...
- Demand 100% test coverage or zero ESLint warnings for personal projects
...
1. **Project** — Name, description, Node.js/TypeScript versions
...
4. **Recommendations** — Should fix for a good first impression (README gaps, missing TSDoc, stale TODOs, mismatched dependencies)
...
OWASP secrets management, SPDX license identifiers, npm packaging, tsconfig.json strict options, .gitignore patterns, semantic versioning
