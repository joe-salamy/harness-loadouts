...
For the first-class automated workflow, prefer `scripts/codex-worktree-flow.py --plan <plan.md>`. That script owns branch creation, worktree creation, implementation, audit, merge, and cleanup. In that script-driven flow, use `implement-worktree` for the implementation stage rather than this manual setup skill.
