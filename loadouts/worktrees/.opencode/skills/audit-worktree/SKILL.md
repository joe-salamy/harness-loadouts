...
If `.codex/handoff/implementation-summary.md` exists, use it as the preferred implementation summary. If the summary is missing, continue with the git diff but state that intent coverage is limited. If the current checkout is `main`, stop before editing and tell the user to run the skill from the implementation worktree.
...
5. Load all skills necessary based on the changed files, risk areas, frameworks, domains, and tooling. Prefer repo-local skills first, then user/global Codex skills when they are relevant. Read each relevant skill's `SKILL.md` directly, then follow its audit guidance. Common mappings:
...
6. If the audit reveals a repeated or reusable workflow gap that existing skills do not cover, create a new skill before finishing the audit:
   - Use the default Codex `skill-creator` skill as the creation workflow.
...
11. Write `.codex/handoff/audit-summary.md` with:
   - Worktree path
   - Branch name
...
12. Final response must briefly restate the audit outcome and point to `.codex/handoff/audit-summary.md`.
